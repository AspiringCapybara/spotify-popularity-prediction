...
- **Pipeline Design and Problem Decomposition**
    ...
    - Structured the project as a simple end-to-end pipeline from raw data to deployable inference: data cleaning → processing → modelling → deployment. 
...
- **Decision-Making Informed by Exploratory Data Analysis (EDA)**
    ...
    - Heavy-tailed behaviour of target variable motivated log-normalisation and model evaluation mainly in log space (rather than raw space).
...
- **Reproducibility and Deployment Awareness (Docker)**
    ...
    - Containerised the application using Docker to ensure reproducible inference behaviour and simplified setup for users and reviewers evaluating the project.
